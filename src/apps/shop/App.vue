...
export default {
  ...
  beforeCreate() {
    this.$store.registerModule(['shop'], { namespaced: true });
    this.$store.registerModule(['shop', 'news'], newsStore);
    this.$store.registerModule(['shop', 'page'], pageStore);
  },
  beforeDestroy() {
    this.$store.dispatch('shop/news/reset');
    this.$store.unregisterModule(['shop', 'news']);
    this.$store.dispatch('shop/page/reset');
    this.$store.unregisterModule(['shop', 'page']);
    this.$store.unregisterModule(['shop']);
  },
  computed: {
    pictures() {
      return {
        '1024w': require('custom/shop/images/header_lg.jpg'),
        '768w': require('custom/shop/images/header_md.jpg'),
        '640w': require('custom/shop/images/header_sm.jpg'),
      };
    },
    application() {
      // eslint-disable-next-line max-len
      return this.$store.getters['applications/application']('shop');
    },
    stories() {
      return this.$store.state.shop.news.cache[0];
    },
    storyCount() {
    ...
    pages() {
      return this.$store.state.shop.page.cache[0];
    },
    pageCount() {
  ...
  methods: {
    async fetchData() {
      ...
      await this.$store.dispatch('shop/news/load', {
        application: this.application.id,
        promoted: true
      });
      await this.$store.dispatch('shop/page/load', {
        application: this.application.id
      });
    }
  }
