...
export default {
  ...
  beforeCreate() {
    this.$store.registerModule(['judo'], { namespaced: true });
    this.$store.registerModule(['judo', 'news'], newsStore);
    this.$store.registerModule(['judo', 'page'], pageStore);
  },
  beforeDestroy() {
    this.$store.dispatch('judo/news/reset');
    this.$store.unregisterModule(['judo', 'news']);
    this.$store.dispatch('judo/page/reset');
    this.$store.unregisterModule(['judo', 'page']);
    this.$store.unregisterModule(['judo']);
  },
  computed: {
    pictures() {
      return {
        '1024w': require('custom/judo/images/header_lg.jpg'),
        '768w': require('custom/judo/images/header_md.jpg'),
        '640w': require('custom/judo/images/header_sm.jpg'),
      };
    },
    application() {
      // eslint-disable-next-line max-len
      return this.$store.getters['applications/application']('judo');
    },
    stories() {
      return this.$store.state.judo.news.cache[0];
    },
    storyCount() {
    ...
    pages() {
      return this.$store.state.judo.page.cache[0];
    },
    pageCount() {
  ...
  methods: {
    async fetchData() {
      ...
      await this.$store.dispatch('judo/news/load', {
        application: this.application.id,
        promoted: true
      });
      await this.$store.dispatch('judo/page/load', {
        application: this.application.id
      });
    }
  }
