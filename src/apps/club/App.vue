...
export default {
  ...
  beforeCreate() {
    this.$store.registerModule(['club'], { namespaced: true });
    this.$store.registerModule(['club', 'news'], newsStore);
    this.$store.registerModule(['club', 'page'], pageStore);
  },
  beforeDestroy() {
    this.$store.dispatch('club/news/reset');
    this.$store.unregisterModule(['club', 'news']);
    this.$store.dispatch('club/page/reset');
    this.$store.unregisterModule(['club', 'page']);
    this.$store.unregisterModule(['club']);
  },
  computed: {
    pictures() {
      return {
        '1024w': require('custom/club/images/header_lg.jpg'),
        '768w': require('custom/club/images/header_md.jpg'),
        '640w': require('custom/club/images/header_sm.jpg'),
      };
    },
    application() {
      // eslint-disable-next-line max-len
      return this.$store.getters['applications/application']('club');
    },
    stories() {
      return this.$store.state.club.news.cache[0];
    },
    storyCount() {
    ...
    pages() {
      return this.$store.state.club.page.cache[0];
    },
    pageCount() {
  ...
  methods: {
    async fetchData() {
      ...
      await this.$store.dispatch('club/news/load', {
        application: this.application.id,
        promoted: true
      });
      await this.$store.dispatch('club/page/load', {
        application: this.application.id
      });
    }
  }
